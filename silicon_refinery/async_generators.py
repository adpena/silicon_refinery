...
async def _compact_history(model: fm.SystemLanguageModel, instructions: str, session: fm.LanguageModelSession) -> fm.LanguageModelSession:
    """
    ...
    """
    logger.info("[SiliconRefinery Stream] Compacting session history to prevent context window explosion...")
    try:
        # We ask the model to summarize its own context
        summary = await session.respond("Please summarize the key entities, rules, and facts from our conversation so far concisely.")
        ...
        logger.warning(f"[SiliconRefinery Stream] History compaction failed: {e}. Falling back to clean session.")
...
async def stream_extract(
    ...
    if is_async:
        async for chunk in iterable:
            session, result = await _process_chunk(model, session, instructions, chunk, schema, history_mode, debug_timing)
...
async def _process_chunk(
    ...
    try:
        ...
        if debug_timing:
            ...
            logger.info(f"[SiliconRefinery Stream] Chunk processed in {elapsed:.3f}s. "
        ...
        if "Context window size exceeded" in error_str or "ExceededContextWindowSizeError" in error_str:
            if history_mode == 'hybrid':
                logger.info("[SiliconRefinery Stream] Context window exceeded in 'hybrid' mode. Clearing history and retrying chunk...")
                session = fm.LanguageModelSession(model=model, instructions=instructions)
                return await _process_chunk(model, session, instructions, chunk, schema, 'clear', debug_timing)
            
            elif history_mode == 'compact':
                # Try to compact the history, then retry
                session = await _compact_history(model, instructions, session)
                return await _process_chunk(model, session, instructions, chunk, schema, 'keep', debug_timing) # retry with keep so we don't loop infinitely if compact fails
